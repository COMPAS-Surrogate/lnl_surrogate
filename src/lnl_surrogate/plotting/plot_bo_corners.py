...
def _make_scipy_result(x, y, space: SearchSpace, model: ProbabilisticModel):
    ...
    min_idx = np.argmin(y)
    return SciPyOptimizeResult(dict(
        fun=y[min_idx],
        x=x[min_idx],
        success=True,
        func_vals=y,
...
def plot_model_partial_dependence(
        in_pts, out_pts, model, search_space: SearchSpace,
        truth: Dict = None,
        **plotting_kwargs
) -> plt.Figure:
    """
...
def plot_evaluations(in_pts, out_pts, model, search_space: SearchSpace, truth: Dict = None) -> plt.Figure:
    """
    Plot the evaluation matrix --> a corner plot of the parameters,
    colored by the order in which they were evaluated.
    """
