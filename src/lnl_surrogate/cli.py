...
def cli_train(
    ...
    if reference_param is None:
        ...
        # update reference_param dict with provided values (if not None)
        reference_param["aSF"] = (
            aSF if aSF is not None else reference_param["aSF"]
        )
        reference_param["dSF"] = (
            dSF if dSF is not None else reference_param["dSF"]
        )
        reference_param["mu_z"] = (
            mu_z if mu_z is not None else reference_param["mu_z"]
        )
        reference_param["sigma_0"] = (
            sigma_0 if sigma_0 is not None else reference_param["sigma_0"]
        )
...
@click.option(
    ...
    required=True,
    type=str,
    help="Output directory of the sampler",
)
...
@click.option(
    "--compas-h5",
    type=str,
    help="Path to COMPAS H5 file (e.g COMPAS_output.h5)",
    required=True,
)
@click.option(
    "--out", type=str, help="Path to output file", default="median_model.png"
)
@click.option(
    "--bilby-result",
    type=str,
    help="Path to bilby result file",
    required=False,
    default=None,
)
...
def cli_plot_median_model(
    compas_h5: str,
    out: str,
    bilby_result: Optional[str] = None,
    aSF: Optional[float] = None,
    dSF: Optional[float] = None,
    mu_z: Optional[float] = None,
